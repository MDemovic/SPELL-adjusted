...
from pysat.solvers import Glucose4, pysolvers # type: ignore
...
def solve(
    ...
    #=>-------------------------------------------------------------------------
    cns, rns = determine_relevant_symbols(A, P, min_pos, size - 1)
    rns = [r for r in rns if not r.endswith("topObjectProperty")]
    sigma = (cns, rns)
    #---------------------------------------------------------------------------
...
def solve_incr(
    ...
    while (
        ...
        i += 1
        dt = time.process_time() - time_start
        print("== Time elapsed: {:.2f}s".format(dt))

    
    print("== Best query found with coverage {}/{}".format(best_coverage, len(P) + len(N)))
    print("== Time elapsed: {:.2f}s".format(dt))
    print(solution2sparql(best_q))
